                            ...
                            @foreach ($transacoes as $transacao)
                                <tr x-sort:item="{{$transacao->id}}" wire:key="tr-transacao-{{ $transacao->id }}" class="border-b dark:border-gray-700 @if($transacao->user->role == 'user' && !$transacao->verificada) bg-red-100 dark:bg-red-900 @endif">
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        ...
                                        <flux:modal name="deletar-transacao-{{$transacao->id}}" class="w-full md:w-96">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg">Apagar transacao</flux:heading>
                                                    <flux:text class="mt-2">Tem certeza que deseja apagar este transacão?</flux:text>
                                                </div>

                                                <div class="flex">
                                                    <flux:spacer />
                                                    <flux:button variant="danger" wire:click="delete({{ $transacao->id }})" class="cursor-pointer">Apagar Transação</flux:button>
                                                </div>
                                            </div>
                                        </flux:modal>
                                    </td>
