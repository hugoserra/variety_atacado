                        ...
                        <thead class="text-xs text-gray-700 dark:text-gray-200 uppercase bg-gray-50 dark:bg-zinc-800 ">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">
                                    <select wire:model.live="qtd_parcelas" class="has-[option.placeholder:checked]:text-zinc-400 dark:has-[option.placeholder:checked]:text-zinc-400 dark:[&>option]:bg-zinc-700 dark:[&>option]:text-white disabled:shadow-none">
                                        <option value="1">Simulação Cartão 1x</option>
                                        <option value="2">Simulação Cartão 2x</option>
                                        <option value="3">Simulação Cartão 3x</option>
                                        <option value="4">Simulação Cartão 4x</option>
                                        <option value="5">Simulação Cartão 5x</option>
                                        <option value="6">Simulação Cartão 6x</option>
                                        <option value="7">Simulação Cartão 7x</option>
                                        <option value="8">Simulação Cartão 8x</option>
                                        <option value="9">Simulação Cartão 9x</option>
                                        <option value="10">Simulação Cartão 10x</option>
                                        <option value="11">Simulação Cartão 11x</option>
                                        <option value="12">Simulação Cartão 12x</option>
                                        <option value="13">Simulação Cartão 13x</option>
                                        <option value="14">Simulação Cartão 14x</option>
                                        <option value="15">Simulação Cartão 15x</option>
                                        <option value="16">Simulação Cartão 16x</option>
                                        <option value="17">Simulação Cartão 17x</option>
                                        <option value="18">Simulação Cartão 18x</option>
                                    </select>
                                </th>
                                <th scope="col" class="px-4 py-3">
                        ...
                        <tbody>
                            @foreach ($produtos as $produto)
                                <tr wire:key="tr-produto-{{ $produto->id }}" class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3">{{ round($produto->getPrecoFinalVendedor() -  $produto->preco_venda_minimo, 2)}}</td>
                                    <td class="px-4 py-3">{{ $produto->getPrecoFinalVendedor() }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex align-center">
                                            <div>de R$ {{$produto->getSimulacaoParcelamentoCartao($produto->getPrecoFinalVendedor(), $qtd_parcelas)}}</div>
                                        </div>
                                    </td>
                               
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <div class="flex">
                                            <flux:button x-on:click="$dispatch('editar-precificacao', {produto_id: {{$produto->id }}})" variant="primary" class="cursor-pointer mr-1">Editar</flux:button>
                                            ...
                                            <flux:modal name="deletar-produto-{{$produto->id}}" class="w-full md:w-96">
                                                <div class="space-y-6">
                                                    <div>
                                                        <flux:heading size="lg">Apagar Precificação</flux:heading>
                                                        <flux:text class="mt-2">Tem certeza que deseja apagar a precificação deste produto?</flux:text>
                                                    </div>

                                                    <div class="flex">
                                                        <flux:spacer />
                                                        <flux:button variant="danger" wire:click="delete({{ $produto->id }})" class="cursor-pointer">Apagar Precificação</flux:button>
                                                    </div>
                                                </div>
                                            </flux:modal>
                                        </div>
