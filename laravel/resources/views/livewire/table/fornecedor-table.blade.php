                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @foreach ($fornecedors as $fornecedor)
                                <tr wire:key="tr-fornecedor-{{ $fornecedor->id }}" class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <flux:button x-on:click="$dispatch('editar-fornecedor', {fornecedor_id: {{$fornecedor->id }}})" variant="primary" class="cursor-pointer mr-1">Editar</flux:button>
                                        <flux:modal.trigger name="deletar-fornecedor-{{$fornecedor->id}}">
                                            <flux:button variant="danger" class="cursor-pointer">Apagar</flux:button>
                                        </flux:modal.trigger>

                                        <flux:modal name="deletar-fornecedor-{{$fornecedor->id}}" class="w-full md:w-96">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg">Apagar Fornecedor</flux:heading>
                                                    <flux:text class="mt-2">Tem certeza que deseja apagar este fornecedor?</flux:text>
                                                </div>

                                                <div class="flex">
                                                    <flux:spacer />
                                                    <flux:button variant="danger" wire:click="delete({{ $fornecedor->id }})" class="cursor-pointer">Apagar Fornecedor</flux:button>
                                                </div>
                                            </div>
                                        </flux:modal>
                                    </td>
